...
class TestCliShutdown(TestCli):
    def test_cli_shutdown_sequence(
        self, mocker: MockerFixture, sample_pdf: str, tmp_path: Path
    ) -> None:
        """Test that the CLI runs the shutdown sequence."""
    ...
    def test_cli_shutdown_with_linger(
        ...
        """Test that the CLI with --linger does not run the shutdown sequence."""
        mock_container_stop = mocker.patch("dangerzone.shutdown.ContainerStopTask.run")
        mock_machine_stop = mocker.patch("dangerzone.shutdown.MachineStopTask.run")
    ...
    def test_cli_shutdown_on_conversion_error(
        self, mocker: MockerFixture, sample_pdf: str, tmp_path: Path
    ) -> None:
        """Test that the CLI runs the shutdown sequence even on conversion error."""
        mocker.patch(
            "dangerzone.logic.Dangerzone.convert_documents",
